...
(function () {
  ...
  fbtns.forEach(function (b) {
    b.addEventListener('click', function () {
      ...
      document.querySelectorAll('#cases .case').forEach(function (c) { c.hidden = !(b.dataset.filter === 'all' || c.dataset.tag === b.dataset.filter); });
    });
  });
  ...
  if (form) {
    ...
    if (form.querySelector('input[name="univers"]')) {
    ...
    } else if (params.get('sujet')) {
      Array.prototype.forEach.call(select.options, function (o) { if (o.textContent === params.get('sujet')) o.selected = true; });
    }
    ...
    form.addEventListener('submit', function (e) {
      ...
      ['prenom', 'nom', 'organisation', 'email', 'telephone', 'objet', 'message'].forEach(function (n) {
        ...
        var v = el ? el.value : '';
        if (n === 'objet') { var u = form.querySelector('input[name="univers"]:checked'); var src = u ? u.value : form.dataset.source; v = (src ? '[' + src + '] ' : '') + v; }
        if (n === 'message') {
          var extra = ['secteur', 'effectif'].map(function (k) { var e = form.querySelector('[name="' + k + '"]'); return e && e.value ? k + ' : ' + e.value : ''; }).filter(Boolean).join(' | ');
          if (extra) v = extra + '\n\n' + v;
        }
